...
namespace Laba1_2Systems_GaussMs_JordanM_LUexpansion
{
    public static class MatricesUtils
    {
        ...
        public static double[,] ReverseMatrix(double[,] matrix)
        {
            ...
            double[,] transposedMatrix = {{matrix[0, 0], matrix[0, 1]}, {matrix[1, 0], matrix[1, 1]}};
            transposedMatrix[0, 1] = -transposedMatrix[0, 1];
            transposedMatrix[1, 0] = -transposedMatrix[1, 0];

            var temp = transposedMatrix[0, 0];
            transposedMatrix[0, 0] = transposedMatrix[1, 1];
            transposedMatrix[1, 1] = temp;
